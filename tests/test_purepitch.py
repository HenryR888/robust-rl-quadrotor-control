'''
Here we execute a pure pitch test using HoverEnv.step()
...
action = np.array([mg, 0.0, tau_0, 0.0])
...
alpha_analytical = np.full_like(t, tau_0/params.I_y)
omega_analytical = (tau_0/params.I_y)*t
phi_analytical = (tau_0/(2*params.I_y))*(t**2)
...
ax.plot(t, alpha_analytical, **EXPECTED, label=rf'$(\tau_0/I_x) = {tau_0/params.I_y:.3f}$ rad/s$^{{2}}$')                                                                                                                    
ax.set_ylabel(r'$\dot{\omega}_x$ (rad/s$^{2}$)', fontsize=9)                                                                                                                                                             
ax.set_title('Roll Angular Acceleration', fontsize=10)                                                                                                                                                              
style_ax(ax)
axes[0].set_ylim(tau_0/params.I_y - 0.01, tau_0/params.I_y + 0.01)
...
plt.tight_layout()
plt.savefig('figures/pure_pitch_test.png', dpi=150, bbox_inches='tight')
plt.show()